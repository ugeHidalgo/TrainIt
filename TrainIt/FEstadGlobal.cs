...
namespace TrainIt
{
    public partial class FEstadGlobal : Form
    {
        ...
        {
            string query;
            laName.Text = "Datos globales almacenados";
            clearData();
        ...
        private void loadDatedData()
        {
            ...
            clearData();
            bool noData = false;
            laName.Text = "Datos comprendidos entre : " + dtpIni.Value.ToShortDateString() + " y " + dtpFin.Value.ToShortDateString();
            //Get global data for the text boxes
            using (SqlConnection conn = new SqlConnection(connString))
            {
                query = @"select
        ...
        private void tsbtGenerales_Click(object sender, EventArgs e)
        {
            loadGeneralData();
        }
